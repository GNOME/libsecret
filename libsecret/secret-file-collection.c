		  ...
		  gpointer user_data)
{
	...
	variant = g_variant_new_from_data (G_VARIANT_TYPE ("(uayuuaya(a{say}ay))"),
					   p,
					   length,
					   TRUE,
					   g_free,
					   contents);
	g_variant_get (variant, "(u@ayuu@ay@a(a{say}ay))",
		       &salt_size, &salt_array, &self->iteration_count,
		       &iv_size, &iv_array, &self->items);
	...
	data = g_variant_get_fixed_array (iv_array, &n_data, sizeof(guint8));
	g_assert (n_data == iv_size);

	self->iv = g_bytes_new (g_variant_get_data (iv_array), iv_size);

	g_task_return_boolean (task, TRUE);

 out:
	g_variant_unref (iv_array);
				...
				GError **error)
{
	...
	n_padded = ((n_data + CIPHER_BLOCK_SIZE) / CIPHER_BLOCK_SIZE) *
		CIPHER_BLOCK_SIZE;
	data = egg_secure_alloc (n_padded + MAC_SIZE);
	...
	if (!calculate_mac (self, data, n_padded, data + n_padded)) {
		egg_secure_free (data);
		g_set_error (error,
			     SECRET_ERROR,
			     SECRET_ERROR_PROTOCOL,
			   ...
			   GError **error)
{
	...
	if (!calculate_mac (collection, data, n_padded, mac)) {
		egg_secure_free (data);
		g_set_error (error,
			     SECRET_ERROR,
			     SECRET_ERROR_PROTOCOL,
	...
	if (memcmp (data + n_padded, mac, MAC_SIZE) != 0) {
		egg_secure_free (data);
		g_set_error (error,
			     SECRET_ERROR,
			     SECRET_ERROR_PROTOCOL,
			      ...
			      gpointer user_data)
{
	...
	salt_array = g_variant_new_fixed_array (G_VARIANT_TYPE_BYTE,
						...
						sizeof(guint8));
	iv_array = g_variant_new_fixed_array (G_VARIANT_TYPE_BYTE,
					      g_bytes_get_data (self->iv, NULL),
					      g_bytes_get_size (self->iv),
					      sizeof(guint8));
	variant = g_variant_new ("(u@ayuu@ay@a(a{say}ay))",
				 g_bytes_get_size (self->salt),
				 salt_array,
