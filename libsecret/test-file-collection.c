...
{
	g_test_trap_subprocess ("/file-collection/concurrent-write/subprocess", 0,
				G_TEST_SUBPROCESS_INHERIT_STDOUT | G_TEST_SUBPROCESS_INHERIT_STDERR);
	g_test_trap_assert_passed ();
}

static void
test_concurrent_write_subprocess (Test *test,
				  gconstpointer unused)
{
...
main (int argc, char **argv)
{
	...
	g_test_add_func ("/file-collection/concurrent-write", test_concurrent_write);
	g_test_add ("/file-collection/concurrent-write/subprocess", Test, NULL, setup_concurrent, test_concurrent_write_subprocess, teardown_concurrent);
