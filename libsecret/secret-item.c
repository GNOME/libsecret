...
static SecretReadableItemInterface *secret_item_readable_item_parent_iface = NULL;
...
static void   secret_item_readable_item_iface      (SecretReadableItemInterface *iface);
...
G_DEFINE_TYPE_WITH_CODE (SecretItem, secret_item, G_TYPE_DBUS_PROXY,
			 ...
			 G_IMPLEMENT_INTERFACE (SECRET_TYPE_READABLE_ITEM, secret_item_readable_item_iface);
...
static void
secret_item_retrieve_secret (SecretItem *self,
			     GCancellable *cancellable,
			     GAsyncReadyCallback callback,
			     gpointer user_data)
{
	...
	secret_item_load_secret (self, cancellable, on_retrieve_load, task);
}

static SecretValue *
secret_item_retrieve_secret_finish (SecretItem *self,
				    GAsyncResult *result,
				    GError **error)
{
...
static void
secret_item_readable_item_iface (SecretReadableItemInterface *iface)
{
	secret_item_readable_item_parent_iface = g_type_interface_peek_parent (iface);
